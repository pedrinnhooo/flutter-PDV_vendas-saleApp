...
class CancelaVenda {
  ...
  CancelaVenda(this._usuario, {Map<String, dynamic> json}) {
    this._json = json;
  ...
  Future<bool> executaCancelaVenda() async {
    if (_movimento.ehfinalizado == 0) {
      bool _result;
      GravaVenda gravaVenda = GravaVenda(this._usuario, json: this._json);
          ...
          if (_movimento.ehorcamento == 0) {
            for (var i=0; i < _movimento.movimentoItem.length; i++){
              ...
              String sql = """SELECT id FROM estoque WHERE (id_pessoa = ${_usuario.idPessoa}) 
                and (id_produto = ${_movimento.movimentoItem[i].idProduto}) 
                and (id_variante ${_movimento.movimentoItem[i].idVariante > 0 ? ' = '+_movimento.movimentoItem[i].idVariante.toString() : 'ISNULL'})""";
              print("sql: "+sql);
