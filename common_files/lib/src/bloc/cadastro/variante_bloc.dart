...
class VarianteBloc extends BlocBase {
  ...
  Stream<List<Variante>> get varianteListOut => varianteListController.stream;
  BehaviorSubject<Variante> varianteController;
  Stream<Variante> get varianteOut => varianteController.stream;
  ...
  Stream<File> get imagemOut => imagemController.stream;

  VarianteBloc(this._hasuraBloc, this.appGlobalBloc){
    variante = Variante();
    varianteDAO = VarianteDAO(_hasuraBloc, appGlobalBloc, variante: variante);
  ...
  getAllVariante() async {
    ...
    VarianteDAO _varianteDAO = VarianteDAO(_hasuraBloc, appGlobalBloc, variante: _variante);
    varianteList = offset == 0 ? [] : varianteList;
  ...
  setCorVariante(Color cor){
    variante.iconecor = cor.toString().replaceAll('Color(', '').replaceAll(")", "").toUpperCase();
    varianteController.add(variante);
  }
  ...
  saveVariante() async  {
    ...
    variante.temImagem = imagem != null ? 1 : 0;
    variante.dataAtualizacao = DateTime.now();
    VarianteDAO varianteDAO = VarianteDAO(_hasuraBloc, appGlobalBloc, variante: variante);
    variante = await varianteDAO.saveOnServer();
    if(variante.temImagem == 1){
  ...
  deleteVariante() async {
    ...
    variante.dataAtualizacao = DateTime.now();
    VarianteDAO _varianteDAO = VarianteDAO(_hasuraBloc, appGlobalBloc, variante: variante);
